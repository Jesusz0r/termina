 ...
 *
 * One validator: Electron's content search and agent-core's JS RegExp fallback
 * both run caller-supplied regular expressions through it, and both must
 * reject nested quantifiers, groups, and backreferences before compiling them.
 * Agent-core's ripgrep path only length-checks: ripgrep itself is linear-time
 * and reports backreferences as an invalid-regex failure. Returns an error
 * message, or null when the pattern is safe to compile.
 */
