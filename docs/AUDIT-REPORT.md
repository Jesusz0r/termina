...
### 5.1. Dual Core Clients Violation (Priority 6)
- **The Rule:** *"electron/worldline-git.ts — only TS client for core/."*
- **The Violation:** [`electron/core-client.ts`](file:///Users/jesusmendoza/Desktop/proyectos/pi-editor/electron/core-client.ts) exists alongside `worldline-git.ts` and directly handles process spawning, protocol framing, and low-level commands. `electron/main.ts` calls `core-client.ts` directly for trust verification and capability tokens, while `electron/worldlines.ts` calls `worldline-git.ts`.
- **Remediation:** Consolidate `core-client.ts` into `worldline-git.ts` as private plumbing. `worldline-git.ts` must be the sole public interface for calling the Rust core daemon.

### 5.2. Parallel `.gitignore` Implementations (Priority 1 & 6)
- **The Rule:** *"One responsibility → one owner. Reuse, extend, or replace — never add a parallel path."*
- **The Violation:**
  1. `core/src/main.rs:10936` implements `op_ls_ignored` using native `git2::Repository::is_path_ignored`.
  2. [`shared/gitignore.ts`](file:///Users/jesusmendoza/Desktop/proyectos/pi-editor/shared/gitignore.ts) implements a custom JavaScript regex-based `.gitignore` parser.
  - This leads to subtle divergences where the Agent runtime (`agent-core`) ignores files that libgit2 tracks (or vice versa), violating deterministic snapshots.
- **Remediation:** Delete `shared/gitignore.ts` and delegate all ignore checks to `core` via `worldline-git.ts` (or share standard ignore match rules).

### 5.3. Dead Backwards-Compatibility Bloat in Sidecar (Priority Rules)
...
- **The Violation:** [`electron/sidecar.ts`](file:///Users/jesusmendoza/Desktop/proyectos/pi-editor/electron/sidecar.ts) contains over 120 lines of legacy file fallback handling (`.segment`, `.legacy-`, `LegacySegmentSource`, ABA hardlink detection) dating back to early prototype sidecar structures. Current writers strictly emit canonical `.sealed` files.
- **Remediation:** Remove all `legacy` branches and shims from `sidecar.ts`, keeping only the single canonical `.sealed` format parser.
...
graph TD
    ...
    subgraph "Phase 2: Performance & Main-Thread Unblocking"
        ...
        P2_3["PTY Egress Micro-Batching & IPC Ack Batching"]
        P2_4["Purge Dead Legacy Sidecar Parsing in sidecar.ts"]
    end

    subgraph "Phase 3: Modular Extraction & Architectural Alignment"
        P3_1["Consolidate core-client.ts into worldline-git.ts"]
        ...
        P3_5["Unify .gitignore: Remove shared/gitignore.ts in favor of core"]
    end
...
1. **Consolidate Core TS Client:** Merge `core-client.ts` into `worldline-git.ts`.
...
5. **Unify Gitignore:** Delete `shared/gitignore.ts` and wire gitignore checks through `worldline-git.ts`.
