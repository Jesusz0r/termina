...
- Extension / hook / plugin **surface** (loader, event bus, user TypeScript). That is a platform. This kernel is one file. Kernel-owned policy is not a plugin. Stdio MCP is a separate, file-configured subprocess client (`agent-core/mcp.ts`), not this plugin surface
...
26. **Summarize is one durable mutation.** Store the handoff and eviction metadata in one `summarize` revision. Replay removes the evicted prefix and inserts that handoff before the surviving tail. This prevents a failed second append from making disk and live history diverge.
27. **A fresh prompt must not append to an old session stream.** Keep the old bundle available for `/resume`. Before the first normal prompt in an empty process, archive a non-empty `current/`, create a fresh active segment, and reset `storageSeq`. `/resume` instead replays every ordered segment and advances `storageSeq` past its maximum.
