...
| 3 | `core/src/main.rs` | 11,210 → 9,603 | Split protocol, Git, snapshot, promotion, and storage internals into Rust modules without moving Git behavior into TypeScript. Done: `trust.rs`, `repo.rs`, `store.rs` (identity/lifecycle/durable files), `store_tx.rs` (lock/journal/recovery). Not separable as leaves: the capture engine shares ~60 descriptor-bound helpers with the promotion-bound ops — that seam needs a bound-fs domain module first, not op-by-op moves. |
