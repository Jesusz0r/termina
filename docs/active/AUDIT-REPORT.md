...
| `agent-core/openai-compat.ts` | 1,862 | Assessed 2026-09-07: canonical protocol-serialization owner (AGENTS.md). 7 importers + dense shared helpers; splitting by protocol family would read as a second protocol mapper. Do not split. Re-verified 2026-09-09: 2 prod + 5 test importers, both body builders single-homed, +38 lines same-responsibility provider work. |
| `electron/session-retention.ts` | 1,622 | Assessed 2026-09-07: single retention-accounting owner (ledger validation, accounting, claims share schema). Do not split. Re-verified 2026-09-09: no ledger outside the owner. |
| `electron/worldline-git.ts` | 1,370 | Assessed 2026-09-07: only public TS client for `core/` (AGENTS.md) — thin typed wrappers, one per op. Splitting violates the single-client rule. Do not split. Re-verified 2026-09-09: no core-protocol bypass, no git CLI outside `core/`. |
| `agent-core/host.ts` | 1,287 | Assessed 2026-09-07: sidecar-exchange owner (AGENTS.md writer); image queue is part of that exchange lifecycle. Do not split. Re-verified 2026-09-09: exchange consumers unchanged (kernel main, request-projection, Electron main). |
| `agent-core/mcp.ts` | 1,256 | Assessed 2026-09-07: single MCP lifecycle owner (config, transport, output share types and one request flow). Do not split. Re-verified 2026-09-09: `mergeClientTools` single-homed, two consumers. |
