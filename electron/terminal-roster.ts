...
export function parseTerminalRoster(raw: unknown): TerminalRosterEntry[] {
  ...
  for (const item of list) {
    ...
    if (typeof rec.id !== "string" || !TERM_ID.test(rec.id) || seen.has(rec.id)) continue;
    ...
    if (typeof rec.sessionId === "string" && isRosterSessionId(rec.sessionId)) entry.sessionId = rec.sessionId;
