...
function splitReadView(content: string): { bodyLines: string[]; marker: string | null } {
  ...
  const page = markerIdx >= 0 ? lines.slice(0, markerIdx) : lines;
  if (page.length === 0 || (page.length === 1 && page[0] === "")) return { bodyLines: [], marker };
  return { bodyLines: page.slice(1), marker };
}
...
function readAllNumbered(root: string, rel: string, source: string): { text: string; rows: string[]; pages: number } {
  ...
  for (;;) {
    ...
    const { bodyLines, marker } = splitReadView(got.content);
    rows.push(...bodyLines);
    for (let i = 0; i < bodyLines.length - 1; i++) acc += `${bodyLines[i]}\n`;
    if (bodyLines.length > 0) acc += bodyLines[bodyLines.length - 1];
    if (!marker) {
      if (source.endsWith("\n")) acc += "\n";
      break;
    }
...
describe("numbered read continuation (#156)", () => {
  ...
  it("advertises an adjacent start_line, not a skipped one", () => {
    ...
    const shown = splitReadView(first.content).bodyLines;
    ...
    expect(splitReadView(cont.content).bodyLines[0]).toBe(rows[nextLine - 1]!);
  });
  ...
  it("reconstructs explicit line ranges", () => {
    const root = project();
    const rows: string[] = [];
    for (let i = 1; i <= 8000; i++) rows.push(`range-${String(i).padStart(4, "0")}`);
    const source = `${rows.join("\n")}\n`;
    ...
    for (;;) {
      ...
      const got = readProjectFile(root, pages === 1
        ? { path: "range.txt", start_line: 1, end_line: 8000 }
...
describe("page emission units (#156)", () => {
  it("emits nothing for empty text", () => {
    expect(emitReadPage("", 100)).toEqual({ body: "", emittedBytes: 0, emittedLines: 0, partial: false });
    expect(emitPlainPage("", 100)).toEqual({ body: "", emittedBytes: 0 });
  });

  it("maps whole lines to source bytes including newlines", () => {
    const page = emitReadPage("ab\ncde\n", 10_000);
    expect(page.body).toBe("ab\ncde");
  ...
  it("falls back to a code-point-safe partial first line", () => {
    const page = emitReadPage(`😀${"x".repeat(100)}`, 3);
    expect(page.partial).toBe(true);
    expect(page.emittedLines).toBe(0);
    expect(page.body).toBe("");
    expect(page.emittedBytes).toBe(0);
    const roomy = emitReadPage(`😀${"x".repeat(100)}`, 5);
    expect(roomy.partial).toBe(true);
    expect(roomy.body).toBe("😀x");
  ...
  it("keeps a newline-only file as a header plus empty line", () => {
    expect(formatReadView("", 1)).toBe("");
    expect(formatReadView("\n", 1)).toBe("line 1\n");
    expect(formatReadView("\n\n", 1)).toBe("lines 1-2\n\n");
    const root = project();
    writeFileSync(join(root, "blank.txt"), "\n");
    const got = readProjectFile(root, { path: "blank.txt" });
    expect(got.isError).toBe(false);
    expect(got.content).toBe("blank.txt line 1\n");
  });
});
