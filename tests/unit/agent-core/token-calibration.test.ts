...
describe("Agent Core Token Calibration Invariants", () => {
  it("passes token calibration tests", async () => {
    const calibration = await import("./token-calibration.ts");
    ...
    const cliBin = new URL("./token-calibration.ts", import.meta.url).pathname;
