...
describe("Agent Core Main Stage 3B Retry Trace Contract", () => {
  ...
  it("retries undici fetch failures and traces provider-network", async () => {
    function readJsonLines(path: string) {
      if (!existsSync(path)) return [];
      return readFileSync(path, "utf8")
        .split("\n")
    ...
    const root = mkdtempSync(join(tmpdir(), "agent-core-main-network-retry-"));
    const events = join(root, "events");
    const terminalId = "term-main-network-retry";
    const requestLog = join(root, "provider-requests.jsonl");
    mkdirSync(events, { recursive: true, mode: 0o700 });
    const providerBase = "https://api.openai.com/v1";
    const mainUrl = new URL("../../../agent-core/main.ts", import.meta.url).href;
    const childScript = `
      ...
      process.argv = [process.execPath, new URL(${JSON.stringify(mainUrl)}).pathname, "-p", "network retry probe"];
      await import(${JSON.stringify(mainUrl)});
    `;
    ...
    try {
      assert.equal(result.code, 0, stderr);
      const requests = readJsonLines(requestLog);
      assert.equal(requests.length, 2, "one network retry must produce exactly two provider requests");
      const sidecar = readJsonLines(join(events, `${terminalId}.jsonl`));
      const settled = sidecar.find((record) => record.t === "agent_settled");
      assert.ok(settled, "run must settle after a recovered fetch failure");
      assert.equal(settled.error ?? null, null);
      const traceDir = join(events, `${terminalId}.traces`);
      const traces = readdirSync(traceDir)
        .filter((name) => /^turn-\d+\.json$/.test(name))
        .sort((a, b) => Number(a.match(/\d+/)?.[0]) - Number(b.match(/\d+/)?.[0]))
      ...
      assert.ok(final);
      assert.equal(final.retryOfAttemptId, retry.attemptId);
      assert.equal(final.fallbackReason, "provider-network");
      assert.equal(final.retryCount, 1);
      console.log("agent-core main network retry trace contract passed");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  }, 60_000);
