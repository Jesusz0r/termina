...
- `electron/agent-activity.ts` — only semantic run-state reducer (`idle` / `working` / `blocked`); main owns wiring and IPC.
- `agent-core/subagents.ts` — background-subagent registry and host contract (spawn validation, claims, handoff/result formats, scanning). Approval/inbox I/O lives in `agent-core/subagents/`. The tool surface stays in `agent-core/main.ts`; the headless entry is `--subagent-task`.
...
- `electron/main.ts` owns project/workspace orchestration and IPC. Main owns authoritative app state; `src/` owns rendering and transient UI state, and requests privileged operations through preload. Keep orchestration out of the core client, `pty-terminal.ts`, and `session-fork.ts`.
