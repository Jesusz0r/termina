...
- `electron/terminal-runtime.ts` — only in-process terminal lifecycle (instance map, PTY spawn/exit, egress ledger, primary events dir + tailer, roster file store, sidecar watch/queue, generation fence, viewer attach/detach); main owns orchestration and IPC validation.
